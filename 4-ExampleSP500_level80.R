setwd("~/Documents/Papers/to_submit/2025-07-30-foundation-ridge2")
...
# Example: Using top 30 S&P 500 stocks from the list
sp500_tickers <- c("NVDA", "AAPL", "MSFT", "AMZN", "GOOGL", "META", "AVGO", 
                   "TSLA", "LLY", "WMT", "JPM", "V", "XOM", "JNJ", 
                   "ORCL", "MA", "MU", "COST", "AMD", "PLTR", "ABBV", "HD", 
                   "BAC", "NFLX", "PG", "CVX", "UNH", "KO", "GE")
...
for (ticker in sp500_tickers) {
  ...
  getSymbols(ticker, src = "yahoo", from = Sys.Date() - 500, to = Sys.Date(), auto.assign = TRUE)
...
for (i in seq_len(ncol(log_returns_ts))) {
  ...
  if (!inherits(fit_ridge, "try-error")) {
    ...
    metrics$ridge2 <- list(
      winkler = misc::winkler_score(test, rescaled_lower, rescaled_upper, 80),
  ...
  if (!inherits(garch_fit, "try-error")) {
    ...
    metrics$rugarch <- list(
      winkler = misc::winkler_score(test, garch_lower, garch_upper, 80),
  ...
  if (!inherits(fgarch_fit, "try-error")) {
    ...
    metrics$fgarch <- list(
      winkler = misc::winkler_score(test, fgarch_lower, fgarch_upper, 80),
